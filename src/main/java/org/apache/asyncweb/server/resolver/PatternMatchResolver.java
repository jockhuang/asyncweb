...
/**
 * A <code>ServiceResolver</code> which maps request URIs to service names. An pattern match is required on a request
 * URI to a service name for returning the matching service. Pattern are in the {@link Pattern} format.<br/>
 * For example : "/hello/.*" will match all the URI begining by the string "/hello/".
 * 
...
public class PatternMatchResolver implements ServiceResolver {
    ...
    /**
     * Adds a mapping from a request URI to a service name. Any existing mapping for the same pattern is overwritten.
     * 
     * @param regexp The regular expression string {@link Pattern}
     * @param serviceName The matching service name
     */
    public void addPatternMapping(String regexp, String serviceName) {
        ...
        if (existingMapping != null) {
            LOG.info("Existing match '{}' replaced by '{}' for pattern '{}'", new Object[] { existingMapping,
                    serviceName, regexp });
        } else {
     ...
     * 
     * @param regexp The regular expression string to remove
     */
    ...
    /**
     * Sets all pattern - service name mappings from a given map. Any existing mappings are removed
     * 
     * @param map The map to set from
     * @throws ClassCastException If any element (key or value) in the map is not a <code>java.lang.String</code>
     */
    ...
    /**
     * Attempts to resolve a service name for the specified request by looking for an existing pattern matching with the
     * URI as the specified request.
     * 
     * @param request The request for which a service name is to be resolved
     * @return The name of the service, or <code>null</code> if no mapping pattern exists for the requests URI
     */

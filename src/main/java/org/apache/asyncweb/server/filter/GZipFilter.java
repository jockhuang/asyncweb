...
/**
 * An HttpServiceFilter compressing data using the gzip algorithm if the client support it.
 * 
...
public class GZipFilter implements HttpServiceFilter {
     ...
     */
    public void handleResponse(NextFilter next, HttpServiceContext context) throws Exception {

        String ae = context.getRequest().getHeader("accept-encoding");
        if (ae != null && ae.indexOf("gzip") != -1 && context.getCommittedResponse() instanceof MutableHttpResponse) {

...
public class SecureRandomKeyFactory implements HttpSessionKeyFactory {
    ...
    /**
     * Creates a session key based on bytes provided from an underlying <code>SecureRandom</code>
     * 
    ...
    /**
     * Sets the number of <i>bytes</i> used when forming keys created by this factory. A hex encoding is applied to the
     * generated key bytes such that the number of <i>characters</i> in keys created by this factory is twice the key
     * length
     * 
    ...
    /**
     * Sets the algorithm employed by the underlying <code>SecureRandom</code>. The default is <code>SHA1PRNG</code>
     * 
    ...
    public void start() {
        ...
        if (LOG.isDebugEnabled())
            LOG.debug("Attempting to obtain SecureRandom using algorithim: " + algorithm);
        try {
            secureRandom = SecureRandom.getInstance(algorithm);
            LOG.info("Using algorithm: " + algorithm);
        } catch (NoSuchAlgorithmException e) {
            LOG.warn("Failed to obtain secure random with algorithm: " + algorithm + ". Resorting to default");
        }

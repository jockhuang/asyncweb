...
/**
 * A <code>ServiceHandler</code> which employs a <code>ServiceResolver</code> to map incoming requests to an
 * <code>HttpService</code> which is then invoked. If an incoming request can not be mapped to an
 * <code>HttpService</code>, a <code>404</code> response status is returned to the client
 * 
 * 
 */
public class HttpServiceHandler implements HttpServiceFilter {
    ...
    /**
     * Adds an <code>HttpService</code> against a service name. The service will be invoked this handlers associated
     * <code>ServiceResolver</code> resolves a request to the specified service name.<br/>
     * 
    ...
    public void addHttpService(String name, HttpService httpService) {
        ...
        if (oldService != null && LOG.isWarnEnabled()) {
            LOG.warn("Duplicate mapping for '" + name + "'. Previous mapping removed");
        }
    ...
    /**
     * Associates this handler with the <code>ServiceResolver</code> it is to employ
     * 
    ...
    /**
     * Attempts to resolve the specified request to an <code>HttpService</code> known to this handler by employing this
     * handlers associated <code>ServiceResolver</code>.<br/>
     * If an <code>HttpService</code> is located for the request, it is provided with the request. Otherwise, a
     * <code>404</code> response is committed for the request
     */
    public void handleRequest(NextFilter next, HttpServiceContext context) throws Exception {
        ...
        if (service == null) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Mapped request [" + request.getRequestUri() + "] to " + "service '" + serviceName + "'");
            }
    ...
    /**
     * Handles a response. This handler does not perform any action for responses - so the specified
     * {@link IoFilter.NextFilter} is invoked immediately.
     */
    ...
    public void start() {
        ...
        for (Entry<String, HttpService> entry : serviceMap.entrySet()) {
    ...
    /**
     * Handles an unmapped request by issuing a <code>404</code> response to the client
     */
    private void handleUnmappedRequest(HttpServiceContext context) {
        HttpRequest request = context.getRequest();
        if (LOG.isWarnEnabled()) {
            LOG.warn("Failed to map '" + request.getRequestUri() + "' to " + "a resource");
        }

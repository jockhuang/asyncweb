...
public class StandardRequestPipeline implements RequestPipeline {
    ...
    /**
     * Releases any requests which can be freed as a result of a request being freed. We simply iterate through the list
     * (in insertion order) - freeing all responses until we arive at one which has not yet been completed
     */
    private void releaseRequests() {
        for (Iterator<Map.Entry<HttpServiceContext, HttpResponse>> iter = entryMap.entrySet().iterator(); iter
                .hasNext();) {

...
/**
 * A <code>TimedPermitIssuer</code> which stores all issued permits in a linked list. As a permit is renewed, its
 * lifetime is extended and it is simply moved to the back of the list (As <code>LinkedPermitIssuer</code> uses a fixed
 * lifetime for all permits and renewals).<br/>
 * Each permit issued by this issuer has direct access to its place in the list - allowing constant time renewals.
 * 
 * 
 */
public class LinkedPermitIssuer implements TimedPermitIssuer {
    ...
    /**
     * The head of the permit list. This entry is the next due to expire
     */
    ...
    /**
     * Creates a <code>LinkedPermitIssuer</code> with a specified lifetime given to new permits. When a permit issued by
     * this issuer is renewed, its expiry time is renewed to the current time plus this lifetime
     * 
    ...
    /**
     * Issues a new <code>TimedPermit</code> for the target object. Unless <code>renew</code>ed, the permit expires
     * after this issuers imposed lifetime. Upon renewal, the permit becomes valid for this issuers configured lifetime
    ...
    /**
     * Determines whether there are any outstanding permits to be processed
     * 
     * @return <code>true</code> if there are no outstanding permits
     */
    ...
    /**
     * Moves a <code>PermitEntry</code> to the back of the list. An entry is moved to the back upon renewal. If we move
     * the current head to the back, we notify to allow the expiry time of the next element to be observed
     * 
    ...
    private class PermitEntry implements TimedPermit {
        ...
        /**
         * Renews this permit. If we are cancelled, no action is taken. Otherwise, our lifetime is extended, and we move
        ...
        /**
         * @return the time remaining, in ms until this permit expires
         */
        long timeToExpiry() {
        ...
        /**
         * @return <code>true</code> if this permit has been cancelled and should be removed
         */
        boolean isCancelled() {
            return isCancelled;
        }
        ...
        /**
         * Cancels this permit. If we are either expired or already cancelled, no action is taken. Otherwise, we mark
         * ourself as cancelled. If we are at the head of the permit list, we notify to allow the notification thread to
    ...
    /**
     * Services the head of the permit list, blocking until work is available. Cancelled and expired entries are removed
     * (listeners are notified of all expired entries)
     * 
     */
    private class ExpiryNotifier implements Runnable {
        ...
        /**
         * Waits for cancellation / expiration of the head entry. If the head entry is expired, listeners are notified.
         * 
         * @return <code>true</code> if the entry is processed, <code>false</code> if we become closed while waiting for
         *         a result
         */
        ...
        /**
         * Examines the head of the list for cancellation or expiry. We block until either the entry is cancelled,
         * expires, or we are closed
         * 
         * @return The previous head entry - if it has expired. <code>null</code> if the entry was cancelled, or we were
         *         closed
         */

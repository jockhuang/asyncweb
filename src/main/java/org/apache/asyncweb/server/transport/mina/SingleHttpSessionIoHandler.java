...
public class SingleHttpSessionIoHandler implements SingleSessionIoHandler {
    ...
    /**
     * Invoked when this connection idles out. If we are in the process of parsing a request, the current request is
     * rejected with a {@link HttpResponseStatus#REQUEST_TIMEOUT} response status.
     */
    ...
    /**
     * Invoked when we fail to parse an incoming request. We configure our parser to discard any further data received
     * from the client, and schedule a response with the appropriate failure code for the current request
     * 
    ...
    /**
     * Invoked when data wrote has been fully written. If we have scheduled closure after sending a final response, we
     * will be provided with the <code>CLOSE_MARKER</code> as our marker object.<br/>
     * This signals us to schedule closure of the connection
     * 
     * @param message The marker provided when writing data. If this is our closure marker, we schedule closure of the
     *            connection
     */
    ...
    private class ContextConverter extends IoFilterAdapter {
        ...
        public void filterWrite(NextFilter nextFilter, IoSession session, WriteRequest writeRequest) throws Exception {
            nextFilter.filterWrite(session,
                    new DefaultWriteRequest(((HttpServiceContext) writeRequest.getMessage()).getCommittedResponse(),
        ...
        public void sessionOpened(final NextFilter nextFilter, final IoSession session) {
            pipeline.setPipelineListener(new RequestPipelineListener() {
                public void responseReleased(HttpServiceContext context) {
                    nextFilter.filterWrite(session, new DefaultWriteRequest(context,
                            ((DefaultHttpServiceContext) context).getWriteFuture()));
                }
            });
    ...
    private class DefaultHttpServiceContext extends AbstractHttpServiceContext {
        ...
        private DefaultHttpServiceContext(HttpRequest request) {
            super((InetSocketAddress) session.getLocalAddress(), (InetSocketAddress) session.getRemoteAddress(),
                    request, container);
        }

...
/**
 * A simple <code>SessionAccessor</code> implementation which acts as a facade to an employed
 * <code>SessionIdentifier</code>, <code>SessionKeyFactory</code> and <code>SessionStore</code>.<br/>
 * 
 * A Default identifier and key factory is employed by this accessor, but the implementations used can be switched (if
 * required) using the appropriate setter methods.<br/>
 * 
 * 
 */
public class DefaultSessionAccessor implements HttpSessionAccessor {
    ...
    /**
     * Sets the <code>SessionIdentifier</code> used for encoding / decoding session keys to / from requests. By default,
     * a <code>CookieIdentifier</code> is employed
     * 
    ...
    /**
     * Sets the <code>SessionKeyFactory</code> employed by this accessor for creating new session keys. By default, a
    ...
    /**
     * Initialises this accessor. If we have not been configured with a session store, we simply employ a
     * <code>BasicSessionStore</code> with a default time out
     */
    public void init() {
        if (store == null) {
    ...
    /**
     * Establishes a new session with the specified response. We employ our <code>SessionKeyFactory</code> to generate a
     * new session key, and request our <code>SessionStore</code> to create a session based on this id.<br/>
     * Our key factory is designed to not provide duplicate keys, but if this occurs we cycle until an unused key is
     * located.
     */

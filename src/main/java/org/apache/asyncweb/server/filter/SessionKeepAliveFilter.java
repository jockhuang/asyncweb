...
/**
 * A <code>ServiceHandler</code> which causes any existing session associated with each request it handles to be
 * renewed. This causes sessions attached to requests to be renewed per-request - even if the request does not cause
 * session access to occur.<br/>
 * 
 * This handler does not need to be installed for deployments which do not employ sessions
 * 
 */
public class SessionKeepAliveFilter implements HttpServiceFilter {
    ...
    /**
     * Handles the specified request. The session associated with the current request - if any - is retrieved - causing
     * its lease to be renewed.
     */

...
public class FileHttpService implements HttpService {
    private static final Logger LOG = LoggerFactory.getLogger(FileHttpService.class);
    ...
    public FileHttpService(String baseUrl, String basePath, String directoryIndexPattern) {
        ...
        if (!(f.isDirectory() && f.exists())) {
            ...
            throw new InvalidParameterException("The base path [ " + basePath + " ] is not a valid path.");
        }
    }
    ...
    public void handleRequest(HttpServiceContext context) throws Exception {
        ...
        if (f.exists() && !f.isDirectory()) {
            ...
            // caching processing
            if (cachingPolicy == null || !cachingPolicy.isCacheable(f, context.getRequest())) {
                response.setHeader("Pragma", "no-cache");
                response.setHeader("Cache-Control", "no-cache");
            } else {
                cachingPolicy.testAndSetCacheHit(f, context.getRequest(), response);
            }

            // setting mime-type based on the mime-map

            String contentType = mimeMap.getContentType(MimeMap.getExtension(f.getName()));

...
/**
 * Defines common sensible behaviour for the rules under which a connection should be kept alive.
 * <code>BasicKeepAliveStrategy</code> will signal that a connection should be closed iff:
 * <ul>
 ...
 * </ul>
 * Implementations should override <code></code> to provide additional constraints
 * 
 */
public class BasicKeepAliveStrategy implements KeepAliveStrategy {
    ...
    /**
     * Determines whether a connection should be kept alive based on a response. This method returns true iff either:
     * <ul>
     * <li>The request is not committed</li>
     * </u> or (in order):
     * <ul>
     * <li>The status of a response does not force closure</li>
     * <li>The request indicates that it wants the connection to remain open</li>
    ...
    public final boolean keepAlive(HttpServiceContext context, HttpResponse response) {
        ...
        if (!context.isResponseCommitted()) {
            ...
            isKeepAlive = !response.getStatus().forcesConnectionClosure() && context.getRequest().isKeepAlive()
                    && doIsKeepAlive(context, response);
            if (!isKeepAlive && LOG.isDebugEnabled() && response.getStatus().forcesConnectionClosure()) {
    ...
    /**
     * Simply returns <code>true</code> by default. Implementations should override this method to provide any

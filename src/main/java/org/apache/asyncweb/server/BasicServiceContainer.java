...
public class BasicServiceContainer implements ServiceContainer {
    ...
    public void setMaxKeepAlives(int maxKeepAlives) {
        ...
        keepAliveStrategy = maxKeepAlives == -1 ? new BasicKeepAliveStrategy() : new CounterKeepAliveStrategy(
                maxKeepAlives);
    }
    ...
    /**
     * Sets all <code>ServiceHandler</code>s employed by this container. Any existing handlers are removed
     * 
    ...
    /**
     * Sets all <code>Transport</code>s employed by this container. Any existing transport are removed
     * 
    ...
    /**
     * Starts our session accessor. If no session accessor has been configured, a default accessor is employed
     */
        ...
        synchronized (transports) {
            for (Transport transport : transports) {
                ...
                try {
                    ...
                    throw new ContainerLifecycleException("Failed to start transport ' " + transport + "'", e);
                }
            }
        }
        if (LOG.isDebugEnabled())

...
public class Main {
    ...
    /**
     * @param asyncWebDir The async web configuration directory - containing the <code>AsyncWeb.xml</code> configuration
     *            file
     * @param serviceConfigName The name of the directory within <code>configDir</code> containing service definitions
     */
    ...
    /**
     * Uses system properties to determine the asyncWeb configuration directory, and (optionally) the service
     * configuration directory contained within
     */
    Main() {
        configDir = System.getProperty(CONFIG_PROPERTY);
        configDir = configDir == null ? "AsyncWeb/" : configDir + "/";
        serviceConfigName = System.getProperty(SERVICE_CONFIG_PROPERTY, DEFAULT_SERVICE_CONFIG);
    }

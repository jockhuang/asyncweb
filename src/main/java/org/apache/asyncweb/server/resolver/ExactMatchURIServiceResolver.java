...
/**
 * A <code>ServiceResolver</code> which maps request URIs to service names. An exact match is required on a request URI
 * to a registered URI must be made for a service name to be resolved
 * 
...
public class ExactMatchURIServiceResolver implements ServiceResolver {
    ...
    /**
     * Adds a mapping from a request URI to a service name. Any existing mapping for the same URI is overwritten.
     * 
     * @param uri The URI
    ...
    public void addURIMapping(String uri, String serviceName) {
        ...
        if (existingMapping != null) {
            LOG.info("Existing service [" + existingMapping + "] replaced by " + "[" + serviceName + "] for URI ["
                    + uri + "]");
        } else {
    ...
    /**
     * Sets all uri - service name mappings from a given map. Any existing mappings are removed
     * 
    ...
    /**
     * Attempts to resolve a service name for the specified request by looking for an existing matching with the same
     * URI as the specified request.
     * 
     * @param request The request for which a service name is to be resolved
     * @return The name of the service, or <code>null</code> if no mapping exists for the requests URI
     */

...
/**
 * A basic <code>Session</code> implementation which holds session values in memory
 * 
 */
class BasicSession implements HttpSession {
    ...
    /**
     * Destroys this session if it is not already destroyed. If destruction takes place, notifications are fired.
     */
    public void destroy() {
        if (destroyIfActive()) {
    ...
    /**
     * Instructs this session to expire. If this session is not already expired, expiration notifications are fired
     */
    void expire() {
        if (destroyIfActive()) {
    ...
    /**
     * Invoked when this session is referenced by a client request. The permit associated with this session is renewed,
     * and subsequent calls to <code>isAttached</code> will return <code>true</code>
     */
    void access() {
        permit.renew();
        isAttached = true;
